...
import { discoverSource } from './source';
...
export function parse(text: string): Program {
  ...
  return withRange({ type: 'Program', body: result.nodes, source: normalized, segments: discoverSource(normalized) }, 0, normalized.length);
}
